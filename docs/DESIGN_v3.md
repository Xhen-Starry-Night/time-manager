...
```
~/.config/time-manager/             # Linux 配置目录（XDG 规范）
├── config.json                     # 全局配置

%APPDATA%\time-manager\             # Windows 配置目录
~/Library/Application Support/time-manager/  # macOS 配置目录
...
例如：data/categories/main/语言/英语/六级/单词-Unit1.json
```
...
| review_records[].memory_quality | String | 记忆表现：重学/困难/好/简单 |
...
```
data/categories/main/语言/英语/六级/单词-Unit1.json
→ path: main/语言/英语/六级/单词-Unit1
```
...
| training_rules[].path_pattern | String | 路径匹配模式（包含分类树名） |
...
| `**` | 匹配任意层级路径 |
| `*` | 匹配单层路径 |
| `main/语言/英语/**` | 匹配 main 分类树下"语言/英语"所有层级 |
| `main/语言/英语/*` | 仅匹配 main 分类树下"语言/英语"下一级 |
| `main/*/六级` | 匹配 main 分类树下"语言/X/六级"（X 为任意值） |
| `work/**` | 匹配 work 分类树下所有层级 |
...
card-create <path>
```
创建学习卡片文件。path 格式为 `<tree>/<category-path>`。

```bash
card-list [path]
```
列出指定路径下的学习卡片。path 格式为 `<tree>/[category-path]`。
...
```bash
card-link <timer-file> <path> --memory-quality <quality>
```
将计时器数据链接到学习卡片，创建复习记录。memory_quality 值为：重学/困难/好/简单。
...
#### 复习看板
```bash
review-list [--urgency <level>] [--search <pattern>]
```
列出待复习卡片，按紧迫度排序。

选项：
- `--urgency`: 紧急程度筛选（all/overdue/three-days/one-week）
- `--search`: 分类树路径模糊搜索
...
│ │  复习英语单词                                            ││
│ │                                                         ││
│ │  2026-06-02 09:00 - 10:30                               ││
│ │  数学期中考试                                            ││
│ │                                                         ││
│ │  2026-06-03 19:00 - 20:00                               ││
│ │  Rust 学习小组                                          ││
│ │                                                         ││
│ └─────────────────────────────────────────────────────────┘│
│                                                             │
...
│ ┌─────────────────────────────────────────────────────────┐│
│ │                                                         ││
│ │  default          默认参数预设    [训练] [查看] [删除]  ││
│ │                                                         ││
│ │  focused          专注学习        [训练] [查看] [删除]  ││
│ │                                                         ││
│ │  relaxed          轻松复习        [训练] [查看] [删除]  ││
│ │                                                         ││
│ └─────────────────────────────────────────────────────────┘│
│                                                             │
├────────────────────────────────────────────────────────────┤
...
| 平台 | 数据目录 | 配置目录 |
|------|----------|----------|
| Linux | `~/.local/share/time-manager/` | `~/.config/time-manager/` |
| Windows | `%LOCALAPPDATA%\time-manager\` | `%APPDATA%\time-manager\` |
| macOS | `~/Library/Application Support/time-manager/` | `~/Library/Application Support/time-manager/` |
