...
**说明**：计时停止后，计时器数据保存到文件，状态恢复为空闲，然后弹出此模态框作为默认跳转。
...
**链接行为说明**：
- 点击计时记录的"链接"按钮，弹出3.2的链接模态框
- 选择或创建卡片后，将计时记录的数据（时长、记忆质量、时间）复制到目标卡片的复习记录中
- 链接完成后，计时记录与目标卡片不再有任何客观联系
- 此后修改计时记录不会影响卡片，修改卡片也不会影响计时记录
- 计时记录仅保存本次学习的基本信息，卡片保存完整的复习记录
